...
export const metadata = { title: "高騰ランキング | CardMarket AI" };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold">高騰ランキング</h1>
          <p className="text-sm text-[#9ca3af]">過去7日間で価格が上昇したカード TOP50</p>
        </div>
      </div>

      <RankingTable entries={entries} type="rise" />
    </div>
  );
}

function RankingTable({ entries, type }: { entries: RankingEntry[]; type: "rise" | "fall" }) {
  if (entries.length === 0) {
    return (
